...
class DQNAgent:
    ...
    def get_epsilon(self, episode, episodes):
        epsilon = min(0.5, (episodes-episode)/episodes)
        return epsilon

    def get_action(self, state, episode, episodes):
        if(episodes == 1):
            self.epsilon = 0

        else:
            self.epsilon = self.get_epsilon(episode, episodes)
...
for episode in range(episodes): #episode 300번 반복
    ...
    while not done: # 종료조건까지
        action = agent.get_action(state, episode, episodes) # epsilon greedy
...
while not done:
    action = agent.get_action(state, 1 , 1)
